...
public class SecurityConfig {
    ...
    public UserDetailsService userDetailsService() {
        ...
        users.add(createUser("physician", physicianPass,
                "PATIENT_READ", "OBSERVATION_WRITE", "CONDITION_WRITE", "ENCOUNTER_WRITE", "MEDICATION_WRITE",
                "ALLERGY_WRITE", "IMMUNIZATION_WRITE", "DIAGNOSTIC_READ"));

        users.add(createUser("nurse", nursePass,
                "PATIENT_READ", "OBSERVATION_WRITE", "CONDITION_READ", "ENCOUNTER_READ", "IMMUNIZATION_WRITE",
                "MEDICATION_READ"));
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                .authorizeHttpRequests(authorize -> authorize
                        // Public Endpoints
                        .requestMatchers("/.well-known/**", "/fhir/metadata", "/actuator/health",
                                "/actuator/prometheus")
                        .permitAll()

                        // --- Patient Resource ---
                        // Registrar can Write (Create/Update), Physician/Nurse can Read
                        .requestMatchers(HttpMethod.POST, "/fhir/Patient").hasAuthority("PATIENT_WRITE")
                        .requestMatchers(HttpMethod.PUT, "/fhir/Patient/**").hasAuthority("PATIENT_WRITE")
                        .requestMatchers(HttpMethod.DELETE, "/fhir/Patient/**").hasAuthority("PATIENT_WRITE")
                        ...
                        .requestMatchers(HttpMethod.POST, "/fhir/Observation").hasAnyAuthority("OBSERVATION_WRITE")
                        .requestMatchers(HttpMethod.GET, "/fhir/Observation/**")
                        .hasAnyAuthority("OBSERVATION_WRITE", "PATIENT_READ", "PATIENT_SELF_READ")

                        // --- Clinical Resources (General) ---
                        .requestMatchers("/fhir/Condition/**").hasAnyAuthority("CONDITION_WRITE", "CONDITION_READ")
                        .requestMatchers("/fhir/Encounter/**").hasAnyAuthority("ENCOUNTER_WRITE", "ENCOUNTER_READ")
                        .requestMatchers("/fhir/AllergyIntolerance/**").hasAnyAuthority("ALLERGY_WRITE", "ALLERGY_READ")
                        ...
                        .requestMatchers("/fhir/Organization/**").hasAnyAuthority("ORGANIZATION_WRITE")
                        .requestMatchers("/fhir/Appointment/**").hasAnyAuthority("APPOINTMENT_WRITE")

                        // --- System Resources ---
                        .requestMatchers("/fhir/Subscription/**").hasAuthority("SUBSCRIPTION_WRITE")
